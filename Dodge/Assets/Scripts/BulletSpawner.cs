...
{
    public GameObject bulletPrefab; // 생성할 탄알의 원본프리맵
    ...
    {
        // timeAfterSpawn 갱신
        timeAfterSpawn += Time.deltaTime;   // Update 실행 사이의 시간 간격을 알기 위해 내장 변수 사용
        ...
        if (timeAfterSpawn >= spawnRate){
            ...
            GameObject bullet = Instantiate(bulletPrefab, transform.position, transform.rotation);  // Instantiate는 복제본을 새엉하고 동시에 생성된 복제본을 메서드 출력으로 반환함
